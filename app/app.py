...
if sites_gdf is not None:
    ...
    for i, row in sites_gdf.iterrows():
        ...
        folium.Marker(
            ...
            tooltip=label,
            icon=folium.DivIcon(
                html=f'<div style="display:flex;align-items:center;white-space:nowrap;"><span style="background-color:white;border:2px solid black;border-radius:50%;width:14px;height:14px;display:inline-block;"></span><span style="margin-left:4px;color:white;font-weight:bold;text-shadow:1px 1px 2px black;">{label}</span></div>',
                icon_size=(150, 20),
                icon_anchor=(7, 10),
            ),
...
with col_center:
  ...
  map_data = st_folium(m, width=None, height=600, use_container_width=True, key="main_map")
